...
refresh_screen (void* arg)
{
  ...
  struct timespec now, timeout, rtime;
  ...
  while (1) {
    ...
    timespec_sub (&rtime, period, &timeout);
...
void
screen_setup (int prio, int col, int lin)
{
  columns = getenv_int ("COLUMNS", col);
  lines = getenv_int ("LINES", lin) / 2;
